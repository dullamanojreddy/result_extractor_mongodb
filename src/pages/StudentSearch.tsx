...
export default function StudentSearch({ role }: { role?: string }) {
  ...
  return (
    <div className="flex-1 flex flex-col justify-between overflow-y-auto bg-white dark:bg-black transition-colors duration-200">
      <div className="p-4 sm:p-6 lg:p-8 max-w-[1600px] w-full mx-auto space-y-6 flex-1 flex flex-col print:hidden">
        
        {/* 1. Header Section */}
        <div className="flex items-center gap-3 sm:gap-4 min-w-0">
          <div className="p-2.5 sm:p-3 bg-purple-500/10 text-purple-600 dark:text-purple-400 rounded-2xl border border-purple-200 dark:border-neutral-800 shrink-0">
            <Search size={28} className="w-6 h-6 sm:w-8 sm:h-8" />
          </div>
          <div className="min-w-0">
            <h1 className="text-xl sm:text-2xl lg:text-3xl font-extrabold text-slate-900 dark:text-white leading-tight">Student Search & Directory</h1>
            <p className="text-slate-500 dark:text-slate-400 text-xs sm:text-sm">Search student records, sort by performance, and view full grade cards</p>
          </div>
        </div>

        {/* 2. Controls Section */}
        <div className="bg-white dark:bg-[#121212] p-4 sm:p-6 rounded-2xl border border-slate-200/85 dark:border-neutral-800 shadow-xs space-y-4">
          {/* Search Input */}
          ...
          <div className="flex flex-col sm:flex-row sm:flex-wrap items-stretch sm:items-center gap-3 sm:gap-4 text-xs font-semibold text-slate-600 dark:text-slate-300">
            {/* Sort Criteria */}
            <div className="flex items-center space-x-2 bg-slate-50 dark:bg-black px-3 py-2 rounded-xl border border-slate-200 dark:border-neutral-800 flex-1 sm:flex-none min-w-0">
              <span className="text-slate-400 flex items-center gap-1 shrink-0"><ArrowDownAz className="w-3.5 h-3.5" /> Sort By:</span>
              <select
                value={sortBy}
                onChange={e => setSortBy(e.target.value as any)}
                className="bg-transparent text-slate-900 dark:text-white font-bold focus:outline-none cursor-pointer w-full"
              >
            ...
            <button
              ...
              className="flex items-center justify-center space-x-1 bg-slate-50 dark:bg-black px-3 py-2 rounded-xl border border-slate-200 dark:border-neutral-800 hover:bg-slate-100 dark:hover:bg-neutral-900 cursor-pointer"
            >
            ...
            {/* Grade filter */}
            <div className="flex items-center space-x-2 bg-slate-50 dark:bg-black px-3 py-2 rounded-xl border border-slate-200 dark:border-neutral-800 flex-1 sm:flex-none min-w-0">
              <span className="text-slate-400 flex items-center gap-1 shrink-0"><SlidersHorizontal className="w-3.5 h-3.5" /> Filter:</span>
              <select
                value={gradeFilter}
            ...
            <div className="text-slate-400 sm:ml-auto font-mono text-[11px]">
      ...
      {isMemoOpen && selectedStudent && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fade-in print:absolute print:inset-0 print:bg-white print:p-0 print:z-50 print:block">
          <div className="bg-white dark:bg-[#121212] border border-slate-200 dark:border-neutral-800 rounded-2xl max-w-3xl w-full shadow-2xl overflow-hidden flex flex-col max-h-[90vh] print:w-full print:max-w-full print:h-screen print:max-h-screen print:border-none print:shadow-none print:rounded-none print:bg-white print:text-black">
            ...
            {/* Modal Header */}
            <div className="px-4 sm:px-6 py-4 border-b border-slate-200 dark:border-neutral-850 bg-slate-50 dark:bg-neutral-900 flex items-center justify-between gap-3 print:hidden">
            ...
            <div className="p-4 sm:p-6 overflow-y-auto space-y-6 flex-1 print:p-0 print:overflow-visible">
              <div className="p-4 sm:p-6 bg-slate-50/50 dark:bg-neutral-900/50 border border-slate-200/50 dark:border-neutral-800 rounded-2xl shadow-2xs space-y-6 print:border-none print:bg-white print:shadow-none print:p-0">
                ...
                {!selectedStudent?.is_missing && selectedStudent.subjects && selectedStudent.subjects.length > 0 && (
                  <div className="space-y-2">
                    ...
                    <div className="overflow-x-auto border border-slate-200 dark:border-neutral-800 rounded-xl bg-white dark:bg-black max-h-52 overflow-y-auto print:max-h-none print:overflow-visible print:border print:border-slate-250">
            ...
            {/* Modal Footer */}
            <div className="px-4 sm:px-6 py-4 border-t border-slate-200 dark:border-neutral-850 bg-slate-50 dark:bg-neutral-900 flex justify-between items-center gap-3 print:hidden">
              <button
                onClick={() => {
                  setIsMemoOpen(false);
                  setSelectedStudent(null);
